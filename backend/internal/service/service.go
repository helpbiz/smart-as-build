...
func (s *Service) GetAvailableRepairRequests(techLat, techLon float64) ([]models.RepairRequest, error) {
	...
	for i := range requests {
		distance := haversineDistance(techLat, techLon, requests[i].Latitude, requests[i].Longitude)
		requests[i].Status = string(rune(int(distance)))
	}
